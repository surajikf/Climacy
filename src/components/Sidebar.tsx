...
export function Sidebar() {
    ...
    // 1. Fetch Sidebar Stats (Fast/Smart)
    useEffect(() => {
        ...
        <div className="w-64 h-screen border-r border-slate-200 bg-slate-50 flex flex-col z-50 sticky top-0 hidden md:flex">
            ...
            <nav className="flex-1 px-4 space-y-1 mt-4 custom-scrollbar overflow-y-auto">
                ...
                {navigation.map((item) => {
                    ...
                    if (item.name === "Settings" && session?.user?.role === "ADMIN") {
                        const adminLink = (
                            <Link
                                key="admin-panel"
                                href="/admin"
                ...
                <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-3">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-500 shrink-0">
                            {session?.user?.role === "ADMIN" ? <ShieldAlert className="w-4 h-4 text-red-500" /> : <UserCircle2 className="w-4 h-4" />}
                        </div>
