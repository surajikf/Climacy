...
export function ClientPickerModal({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="bg-white w-full max-w-4xl rounded-2xl border border-slate-200 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-slate-900 tracking-tight">
                                {mode === "oversight" ? "Choose Recipients" : "Choose Sample Client"}
                            </h3>
                ...
                <div className="p-4 border-b border-slate-100 bg-white sticky top-0 z-10 space-y-3">
                    <div className="relative group">
                        ...
                        <input
                            type="text"
                            placeholder="Search name, industry, or contact"
                            value={searchQuery}
                    ...
                    {mode === "oversight" && (
                        <div className="space-y-2">
                            <div className={cn("grid gap-2", showActivityFilters ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-2")}>
                                <button
                                    type="button"
                                    onClick={selectAll}
                                    className="px-2.5 py-2 text-xs font-bold rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                                >
                                    Include All
                                </button>
                                <button
                                    type="button"
                                    onClick={excludeAll}
                                    className="px-2.5 py-2 text-xs font-bold rounded-lg border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100 transition-colors"
                                >
                                    Exclude All
                                </button>
                                {showActivityFilters && (
                                    <>
                                        <button
                                            ...
                                            className="px-2.5 py-2 text-xs font-bold rounded-lg border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Active Only
                                        </button>
                                        <button
                                            ...
                                            className="px-2.5 py-2 text-xs font-bold rounded-lg border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Inactive Only
                                        </button>
                                    </>
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50/30">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center space-y-2">
                            ...
                            <p className="text-sm text-slate-500 max-w-[280px]">Try another search, or close this popup and change audience settings.</p>
                        </div>
                    ) : (
