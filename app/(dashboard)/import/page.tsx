...
export default function ImportIntegrationsPage() {
    ...
    const handleGmailSync = async (accountId: string, accountName: string, accountEmail?: string) => {
        ...
        try {
            ...
            const result = await safeImportRequest<{ jobId: string }>("/api/import/gmail", {
                ...
                body: JSON.stringify({ accountId, options: syncOptions })
            });
    ...
    const saveActiveProfile = () => {
        ...
        saveProfileForAccount(activeGmailAccountForProfile.email.toLowerCase(), gmailSyncProfile);
    ...
    const handleAddGmailAccount = () => {
        ...
        const url = `${appPath("/api/gmail/connect")}?label=${encodeURIComponent(label)}&intent=${encodeURIComponent(gmailIntent)}&returnTo=${encodeURIComponent("/import")}`;
                        ...
                        <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Authorization</h4>
                                </div>
                                <div className="p-4 bg-orange-50 rounded-2xl border border-orange-100 flex flex-col items-center gap-4 text-center">
                                    <div className="space-y-1">
                                        <p className="text-[11px] font-bold text-orange-900">
                                            {zohoConfig.hasRefreshToken ? "✓ Zoho Access Authorized" : "OAuth Synchronization Required"}
                                        </p>
                                        <p className="text-[9px] text-orange-700/70 font-medium leading-relaxed">
                                            {zohoConfig.hasRefreshToken 
                                                ? "Your account is linked with Zoho Bigin. You can re-authorize if you need to refresh permissions." 
                                                : "Authorize this application to securely synchronize your deals and contacts from Zoho Bigin CRM."}
                                        </p>
                                    </div>
                                    <button
                                        onClick={handleZohoAuth}
                                        className="w-full h-10 bg-orange-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-orange-700 transition-all active:scale-[0.98] shadow-md shadow-orange-600/10 flex items-center justify-center gap-2"
                                    >
                                        <Cloud className="w-3.5 h-3.5" />
                                        {zohoConfig.hasRefreshToken ? "Re-Authorize Zoho" : "Begin Zoho Sync"}
                                    </button>
                                </div>
                            </div>
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    {[
                                        { id: "send", title: "Send Emails", detail: "Best for campaigns and test dispatch." },
                                        { id: "sync", title: "Sync Contacts", detail: "Best for importing contacts only." },
                                        { id: "both", title: "Do Both", detail: "Connect once for sending and sync." },
                                    ].map((item) => (
                                        <button
                                            key={item.id}
                                            type="button"
                                    ...
                                    <div className="grid sm:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                placeholder="noreply.com, spam.com"
                                                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 outline-none focus:border-blue-500"
                                            />
                                        </div>
                                        <div>
