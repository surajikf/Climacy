...
const ClientRow = memo(({ contact, index, page, pageSize, onEdit, onDelete, onToggleBlock, isSimpleGmailView = false, showServicesColumn = true }: any) => {
    ...
    return (
        ...
        >
            <td className="px-4 py-5 w-12 text-center text-slate-400 text-xs font-bold align-top sticky left-0 bg-white z-[1]">
                <span className="text-slate-500">{(page - 1) * pageSize + index + 1}</span>
            </td>
            <td className="px-6 py-5 align-top sticky left-12 bg-white z-[1]">
            ...
            <td className="px-6 py-5 align-top">
                <div 
                    ...
                    title={
                        contact.relationshipLevel === "Active" ? "Active customer: Ongoing services or recent invoices. High follow-up priority." :
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                toast.error(result.error?.message || "Failed to remove client.");
            }
        } catch (err) {
            console.error(err);
        } finally {
            ...
            {view === "clients" || view === "rolebased" ? (
                <>
                    ...
                    <div className="bg-white overflow-hidden rounded-3xl border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-sm">
                        {isSimpleGmailView && (
                            <div className="px-6 py-3 bg-blue-50 border-b border-blue-100">
                                <p className="text-[11px] font-semibold text-blue-800">
                                    Gmail contacts do not include billing/service mapping.
                                </p>
                            </div>
                        )}
                        {!isSimpleGmailView && hasAnyGmailRows && (
                            <div className="px-6 py-3 bg-amber-50/70 border-b border-amber-100 flex flex-wrap items-center gap-x-4 gap-y-1">
                                <p className="text-[11px] font-semibold text-amber-800">
                                    {gmailRowsWithoutInvoiceServices} Gmail contacts in this view have no billing/service mapping.
                                </p>
                        ...
                        <div className="px-6 py-2 border-b border-slate-100 bg-slate-50/50 md:hidden">
                            <p className="text-[11px] font-medium text-slate-600">Scroll horizontally to view all columns.</p>
                        </div>
                        <div className="overflow-x-auto overscroll-x-contain">
                        <table className="w-full min-w-[1200px] text-left border-collapse overflow-hidden">
                            <thead>
                                <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] sm:text-[11px]">
                                    {isSimpleGmailView ? (
                                        ...
                                        <>
                                            <th className="px-4 py-6 font-black uppercase tracking-[0.2em] text-slate-400 w-12 text-center sticky left-0 bg-slate-50/95 z-10">Sr.</th>
                                            <th className="px-6 py-6 font-black uppercase tracking-[0.2em] text-slate-400 w-[30%] sticky left-12 bg-slate-50/95 z-10">Client</th>
                                            {showServicesColumn && <th className="px-6 py-6 font-black uppercase tracking-[0.2em] text-slate-400 w-[15%]">Service</th>}
                                            ...
                                            <th className="px-6 py-6 font-black uppercase tracking-[0.2em] text-slate-400 w-[10%]">Relationship</th>
                                            <th className="px-6 py-6 font-black uppercase tracking-[0.2em] text-slate-400 w-[10%]">Source</th>
