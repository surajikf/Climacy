...
export async function GET(req: Request) {
    try {
        ...
        if (!settings || (!process.env.ZOHO_CLIENT_ID && !settings.zohoClientIdEncrypted) || (!process.env.ZOHO_CLIENT_SECRET && !settings.zohoClientSecretEncrypted) || !zohoConnection?.refreshTokenEncrypted) {
            return error("NOT_CONFIGURED", "Zoho settings not fully configured.");
        }

        console.log("[ZOHO_STAGES] Decrypting credentials...");
        const clientId = process.env.ZOHO_CLIENT_ID || decrypt(settings.zohoClientIdEncrypted);
        const clientSecret = process.env.ZOHO_CLIENT_SECRET || decrypt(settings.zohoClientSecretEncrypted);
