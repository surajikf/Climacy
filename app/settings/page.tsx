...
export default function SettingsPage() {
    ...
    const handleTestNode = async (type: "GMAIL" | "SMTP", accountId?: string) => {
        const resolvedAccountId =
            type === "GMAIL"
                ? (accountId
                    || formData.gmailAccounts.find((a: any) => a.isDefault)?.id
                    || formData.gmailAccounts.find((a: any) => a.scopeGranted)?.id
                    || formData.gmailAccounts[0]?.id)
                : undefined;
        const id = resolvedAccountId || type;
        setNodeTestLoading(id);
        try {
            const res = await fetch(apiPath("/settings/test-dispatch"), {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ nodeType: type, accountId: resolvedAccountId })
            });
            const data = await res.json();
            if (data.success) {
                toast.success("Email Test Sent", {
                    description: `A test email was sent to ${session?.user?.email}.`,
            ...
            toast.error("Network Error", { description: "Could not reach the email connection test endpoint." });
        } finally {
    ...
    if (loading) return <SmartLoader label="Loading Settings" description="Fetching current configuration..." />;
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="w-full space-y-8 pb-20 animate-in fade-in duration-500 px-3 sm:px-4 lg:px-6">
            <PageHeader
                title="Settings"
                subtitle="Manage AI, sending channels, and workspace configuration."
            ...
            <div className="grid gap-6">
                ...
                <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-8">
                    <div className="flex items-center gap-3 pb-4 border-b border-slate-50">
                        ...
                        <div>
                            <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">AI Settings</h3>
                            <p className="text-xs font-medium text-slate-400">Configure AI provider and API access.</p>
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-slate-50 p-6 rounded-lg border border-slate-100 flex flex-col justify-center">
                            {formData.aiProvider === "Groq" ? (
                                <CredentialInput label="Groq API Key" value={formData.groqApiKey} field="groqApiKey" placeholder="gsk_••••••••" />
                            ) : (
                            ...
                            <p className="text-[10px] text-slate-400 font-medium mt-3 flex items-center gap-1.5 uppercase tracking-tight italic">
                                ...
                                Encrypted secret storage active.
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-2xl bg-white border border-slate-200 shadow-sm relative overflow-hidden group">
                                    <div className="space-y-1 relative z-10">
                                        ...
                                        <p className="text-xs text-slate-500 font-medium">Use for high-volume delivery providers (Amazon SES, Postmark, etc.)</p>
                                    </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-8 items-start">
                                    ...
                                    <div className="space-y-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                                        ...
                                        <CredentialInput label="Sender Display Name" field="smtpSenderName" value={formData.smtpSenderName} placeholder="Project Name" type="text" />
            ...
            {resetModalOpen && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl border border-slate-200 animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-lg font-black text-slate-900 mb-2 uppercase tracking-tight leading-none">Reset All Settings?</h3>
                        <p className="text-sm text-slate-500 font-medium leading-relaxed mb-8">This action restores default settings and removes saved API keys and sending account configuration.</p>
