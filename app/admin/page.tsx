    ...
    const handleAction = async (
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.error(data.error?.message || data.error || "Failed to update user access.");
            }
        } catch (error) {
            toast.error("Network error while updating access.");
        }
    };
        ...
        <div className="w-full space-y-8 animate-in fade-in duration-500 pb-20 px-3 sm:px-4 lg:px-6">
            <header className="px-2">
                <div className="flex items-center gap-3 text-red-600 mb-2">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">Admin Area</span>
                </div>
                <h2 className="text-3xl font-bold tracking-tight text-slate-900">Access Management</h2>
                <p className="text-slate-500 font-medium text-sm mt-1">Manage user roles, approval status, and data access.</p>
            </header>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                                ...
                                <td className="px-6 py-4">
                                    ...
                                    <div className="mt-2">
                                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${userItem.canAccessInvoiceData ? "bg-blue-50 text-blue-700 border-blue-100" : "bg-slate-50 text-slate-500 border-slate-200"}`}>
                                            Billing Data: {userItem.canAccessInvoiceData ? "Enabled" : "Disabled"}
                                        </span>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                        {userItem.status === "APPROVED" && !userItem.canAccessInvoiceData && (
                                            <button onClick={() => handleAction(userItem.id, "GRANT_INVOICE_ACCESS")} className="px-3 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg transition-colors text-xs font-semibold" title="Grant Billing Data Access">
                                                Grant Billing Access
                                            </button>
                                        )}
                                        {userItem.status === "APPROVED" && userItem.canAccessInvoiceData && userItem.email !== currentUser?.email && (
                                            <button onClick={() => handleAction(userItem.id, "REVOKE_INVOICE_ACCESS")} className="px-3 py-2 bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-lg transition-colors text-xs font-semibold" title="Revoke Billing Data Access">
                ...
                {users.length === 0 && !loading && (
                    <div className="p-8 text-center text-sm font-medium text-slate-400">No users found.</div>
                )}
            </div>
        </div>
    );
