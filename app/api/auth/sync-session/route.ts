...
export async function POST(request: Request) {
    ...
    try {
        ...
        const hasExistingRefreshToken = Boolean(existingAccount?.refreshTokenEncrypted);

        // A usable refresh token is mandatory for send-capable Gmail integration.
        // NextAuth may not provide refresh token on repeat logins, so we accept either:
        // - newly provided refresh token, or
        // - already stored refresh token for this account.
        if (!safeRefreshToken && !hasExistingRefreshToken) {
            const res = ok({
                synced: false,
        ...
        const upsertData: any = {
            ...
            lastStatus: hasSmtpPermission ? "HEALTHY" : "RECONNECT_REQUIRED",
            lastUsed: hasSmtpPermission ? new Date() : null,
            updatedAt: new Date(),
        };
