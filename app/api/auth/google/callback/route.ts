...
export async function GET(request: Request) {
    try {
        ...
        if (!code) {
            return NextResponse.json({ error: "No authorization code received from Google." }, { status: 400 });
        }
        ...
        if (!tokenResponse.ok) {
            ...
            return NextResponse.json({ error: tokens.error_description || "Google token exchange failed." }, { status: 500 });
        }
