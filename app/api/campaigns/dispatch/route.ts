...
export async function POST(request: Request) {
    try {
        ...
        if (!parsed.success) {
            return error("VALIDATION_ERROR", "Campaign ID is required.", {
                status: 400,
        ...
        if (!campaign || !campaign.client) {
            return error("NOT_FOUND", "Campaign client record not found.", {
                status: 404,
            });
        }
        ...
        if (!result.success) {
            return error("INTEGRATION_ERROR", result.error || "Sending failed.", {
                status: 400,
            });
        }
        ...
        return error(
            "INTERNAL_ERROR",
            err.message || "Dispatch failed due to an internal error.",
        );
    }
}
