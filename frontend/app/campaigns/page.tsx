...
import { readCampaignSession, writeCampaignSession } from "@/frontend/lib/campaign-session";
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-1.5 relative">
                                            <div className="flex items-center justify-between pl-1">
                                                ...
                                                <button onClick={handleSuggestSubjects} disabled={isGeneratingSuggestions} className="flex items-center gap-1.5 text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest bg-blue-50 px-2 py-1 rounded transition-all">
    ...
    return (
        <div className="w-full pb-14 md:pb-20 px-3 sm:px-4 lg:px-6">
            <div className="mb-6 md:mb-8">
                <h2 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-900">Campaign Builder</h2>
                ...
                <div className="flex-1 space-y-5 md:space-y-6 lg:space-y-8 min-w-0 w-full">
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="px-4 sm:px-5 md:px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                            <h3 className="text-base font-bold text-slate-900 tracking-tight uppercase">0. Select Audience Source</h3>
                        </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className={cn("p-4 sm:p-5 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4", audienceSources.length === 0 && "opacity-50 pointer-events-none select-none")}>
                            {campaignTypes.filter((type) => !(type.id === "Reactivation" && !hasInvoiceSelected)).filter((type) => !(type.id === "Cross-Sell" && !hasInvoiceSelected)).map((type) => (
                                <div key={type.id} onClick={() => setSelectedType(type.id)} className={cn("p-5 rounded-lg border cursor-pointer transition-all", selectedType === type.id ? "bg-blue-50/50 border-blue-500 ring-1 ring-blue-500" : "bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50")}>
                                    <div className="flex items-start justify-between mb-2">
                                        <div className="flex items-center gap-3">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div><h4 className="text-[10px] font-black text-slate-700 uppercase tracking-[0.1em]">Audience Segmentation</h4><p className="text-[9px] text-slate-400 font-black uppercase tracking-wider">Pick service users for this campaign</p></div>
                                        </div>
                                        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
                                            <button onClick={() => setServiceLogic('OR')} className={cn("px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all", serviceLogic === 'OR' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700")}>Match Any</button>
                                            <button onClick={() => setServiceLogic('AND')} className={cn("px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all", serviceLogic === 'AND' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700")}>Match All</button>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                        {services.map(s => (
                                            <div key={s.id} onClick={() => toggleService(s.serviceName)} className={cn("px-3 py-2 rounded-lg border-2 cursor-pointer transition-all flex items-center gap-2", selectedServices.includes(s.serviceName) ? "bg-blue-50 border-blue-500/30 text-blue-700 shadow-sm" : "bg-white border-slate-100 text-slate-500 hover:border-slate-200")}>
                                                ...
                                                <span className="text-[9px] font-black uppercase tracking-wider truncate">{s.serviceName}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="px-4 sm:px-5 md:px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                            <h3 className="text-base font-bold text-slate-900 tracking-tight uppercase">2. Sample Email</h3>
                        </div>
                        <div className={cn("p-4 sm:p-5 md:p-6 space-y-5 md:space-y-6", audienceSources.length === 0 && "opacity-50 pointer-events-none select-none")}>
                             <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] pl-1">Master Subject Line</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] pl-1">Master Email Body (Your Sample)</label>
                ...
                <div className="w-full lg:w-[22rem] 2xl:w-96 flex-shrink-0 lg:sticky lg:top-6 space-y-4 md:space-y-6">
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="px-4 sm:px-5 md:px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2">
                            <Network className="w-4 h-4 text-blue-600" /><h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Pre-Flight Check</h3>
                        </div>
                            ...
                            <div>
                                ...
                                {audienceSources.length > 0 && selectedType && !loadingAudience && audienceData.count > 0 && (
                                    <p className="mt-2 text-xs text-slate-600 flex items-center justify-between">
                                        ...
                                        <button onClick={() => { fetchTargetClients(); setShowOversightModal(true); }} className="text-blue-600 hover:text-blue-700 font-bold uppercase tracking-tighter text-[10px]">View/Edit List</button>
                                    </p>
                                )}
                            </div>
                            <button onClick={() => handleGenerateSample()} disabled={!isReady || isGenerating || audienceData.count === 0} className="w-full bg-blue-600 text-white py-4 px-4 rounded-xl text-sm font-black hover:bg-blue-700 transition-all flex items-center justify-center gap-3 shadow-lg disabled:opacity-40 group">
