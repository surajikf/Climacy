...
async function resolveGmailAuth(
    ...
    const hasOAuthClient = credentialCandidates.length > 0;
    const hasAnyLinkedAccount = Boolean(selectedIdentity);
    const hasSenderEmail = Boolean(user);
    const hasRefreshToken = Boolean(refreshToken);

    if (!hasOAuthClient || !hasSenderEmail || !hasRefreshToken) {
        const reasons: string[] = [];
        if (!hasOAuthClient) reasons.push("Google OAuth client is not configured in Settings");
        if (!hasAnyLinkedAccount) reasons.push("No Gmail account is connected for this user");
        if (hasAnyLinkedAccount && !hasRefreshToken) reasons.push("Connected Gmail account is missing a refresh token (reconnect required)");
        if (hasAnyLinkedAccount && !hasSenderEmail) reasons.push("Connected Gmail account email is missing");

        const reasonText = reasons.length > 0 ? reasons.join("; ") : "Gmail configuration is incomplete";
        console.warn(`[MAIL] Gmail configuration check failed: ${reasonText}`);
        return { success: false, error: reasonText };
    }
    ...
    const gmailUser = user as string;
...
export async function sendStrategicEmail(options: MailOptions, dispatchOptions?: DispatchOptions) {
    ...
    const primaryProvider = dispatchOptions?.forceProvider || settings.emailProvider;
    const autoFailover =
        dispatchOptions?.disableFailover === true
            ? false
            : (settings as any).autoFailover !== false; // Default true

    console.log(`[MAIL] Sending with provider: ${primaryProvider} (Failover: ${autoFailover ? 'ON' : 'OFF'})`);
    ...
    if (primaryProvider === "GMAIL") {
        ...
        // If Gmail fails and failover is enabled, try SMTP fallback.
        if (!result.success && autoFailover) {
            console.warn(`[MAIL] Primary Gmail send failed: ${result.error}. Trying SMTP fallback...`);
...
async function sendViaSMTP({ to, subject, text, html }: MailOptions, settings: any) {
    try {
        ...
        const diagnosticHtml = (html || "") + `<!-- Provider: SMTP | Sender: ${smtpHost} -->`;
        ...
        console.log("[MAIL] SMTP email sent:", info.messageId);
...
async function sendViaGmail(
    ...
    try {
        const diagnosticHtml = (html || "") + `<!-- Provider: GMAIL | Sender: ${auth.gmailUser} -->`;
